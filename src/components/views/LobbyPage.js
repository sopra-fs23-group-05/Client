...
const Lobby = () => {
  ...
  const changeTeam = (teamNr, type) => {
    ...
    teamWebSocket.current.send(
      JSON.stringify(
        new TeamRequest(
          parseInt(window.location.href.slice(-6), 10),
          teamNr,
          parseInt(userId, 10),
          type
        )
  ...
  useEffect(() => {
    teamWebSocket.current.onmessage = (event) => {
      ...
      if (IncomingMessage.type === "addition") {
        ...
        if (IncomingMessage.teamNr === 1) {
          lobby.team1 = lobby.team1.filter(
            (user) => user.username !== IncomingMessage.username
          );
          const newTeam1Members = team1Members.filter(
            (member) => member.username !== IncomingMessage.username
          );
          setTeam1Members(newTeam1Members);
        } else if (IncomingMessage.teamNr === 2) {
          lobby.team2 = lobby.team2.filter(
            (user) => user.username !== IncomingMessage.username
          );
          const newTeam2Members = team2Members.filter(
            (member) => member.username !== IncomingMessage.username
          );
  ...
  const team1Content = team1Members.map((user) => (
    <div key={user.username} className="team-member">
      {user.username}
    </div>
  ));
      ...
      <div className="horizontal-box" style={{ position: "fixed", top: 0 }}>
        ...
        <div className="flex-container">
          <div className="buttonPanel">
            ...
            <ul className="team-member-box">{team1Content}</ul>
            <Button
              variant="contained"
              className="buttonLogin"
          ...
          <div className="buttonPanel">
            ...
            <ul className="team-member-box">{team2Content}</ul>
            <Button
              variant="contained"
              className="buttonLogin"
