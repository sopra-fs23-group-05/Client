...
export default function Game() {
    ...
    async function fetchData() {
      try {
        const responseRole = await api.get(
          `/games/${accessCode}/users/${playerName}`
        );
  ...
  let [displayedCard, setCard] = useState(
    new Card({
      word: "Loading...",
      taboo1: "Loading...",
      taboo2: "Loading...",
  ...
  useEffect(() => {
    ...
    webSocket.current = new WebSocket(
      "wss://sopra-fs23-group-05-server.oa.r.appspot.com/chat"
    );
  ...
  useEffect(() => {
    ...
    // Activate the following line for deployment.
    cardWebSocket.current = new WebSocket(
      "wss://sopra-fs23-group-05-server.oa.r.appspot.com/cards"
    );
    // Activate the following line for local testing.
  ...
  const listChatMessages = chatMessages.map((ChatMessage, index) => (
    <Box
      ...
      sx={{
        display: "flex",
        flexDirection:
          ChatMessage.type === "description" ? "row" : "row-reverse",
        width: "100%",
  ...
  let sendFields = (
    ...
    >
      <TextField
        className={"textField-chat-input"}
          ...
          <Button
            ...
            onClick={async () => {
              const response = await fetch(
                `https://api.datamuse.com/words?sp=${displayedCard.word}&md=d`
              );
